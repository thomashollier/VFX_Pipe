...
def smooth_gaussian(im:np.ndarray, sigma) -> np.ndarray:
	...
	print("\tApplying smooth gaussian")
...
def computeNormal(gradient_x:np.ndarray, gradient_y:np.ndarray, intensity=1):
...
def computeAmbocc(im:np.ndarray):
	
	print("\tComputing ambocc map...")
	shadowStrength = .5
...
def generateOutputFilename(Org_Path:str,addto:str):
	'''
	...
	oldPath = Org_Path
	mapType = addto
	fileType = oldPath.split(".")[-1:][0]
	if len(oldPath.split(".")) > 2:
		tileNumb = "%s." % oldPath.split(".")[1]
		texName =  oldPath.split(".")[0]	
	else:
		tileNumb = "" 
		texName =  oldPath.split(".")[0]	

	texName = texName.replace("bump", mapType)
	newPath = "%s.%s%s" % (texName, tileNumb, fileType)
	return newPath
...
def Convert(input_file,smoothness,intensity):
	...
	normalMap = computeNormal(sobel_x, sobel_y, intensity)
	normalMapFile = generateOutputFilename(input_file,"normal")
	pyplot.imsave(normalMapFile, normalMap)
	print("\t--- Saved")

	flipgreen(normalMapFile)

	amboccMap = computeAmbocc(im)

	amboccMapFile = generateOutputFilename(input_file,"ambocc")
	pyplot.imsave(amboccMapFile,amboccMap)
	print("\t--- Saved")
	Cleanupambocc(amboccMapFile)
...
for i in os.listdir():
	if i.find("ampartA_v02_bump.png")>0:
		Convert(i, 2,1)
